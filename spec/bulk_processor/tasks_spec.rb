...
describe 'bulk_processor namespace' do
  ...
  describe 'bulk_processor:split' do
    let(:job) { instance_double(BulkProcessor::Job::SplitCSV, perform: true) }

    before(:all) { Rake::Task.define_task(:environment) }

    before { allow(BulkProcessor::Job::SplitCSV).to receive(:new).and_return(job) }

    it 'starts the job' do
      expect(job).to receive(:perform)
        .with('MockCSVProcessor', '{"foo":"bar"}', 'file.csv', '2')
      Rake::Task['bulk_processor:split'].reenable # allows multiple calls
      rake_cmd =
        'bulk_processor:split[MockCSVProcessor,{"foo":"bar"},file.csv,2]'
      Rake.application.invoke_task(rake_cmd)
    end
  end
end
