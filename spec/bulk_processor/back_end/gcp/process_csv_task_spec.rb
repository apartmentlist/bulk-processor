require 'bulk_processor/back_end/gcp/process_csv_task'
...
describe 'bulk_processor namespace' do
  describe 'bulk_processor:start' do
    ...
    let(:rake_cmd) do
      'bulk_processor_gcp_pods:start[MockCSVProcessor,foo=bar,file.csv]'
    end

    before(:all) { Rake::Task.define_task(:environment) }

    before do
      Rake::Task['bulk_processor_gcp_pods:start'].reenable # allows multiple calls
